...
class WrappedDatabase extends React.Component {
    ...
    applySort() {
        ...
        const sortFieldType = activeTable.allFields[sortField].fieldType;
        data.sort((a, b) => {
            let aa = a;
            let bb = b;
            ...
            switch (sortFieldType) {
                case 'F':
                    if (Number(aa.fields[sortField]) > Number(bb.fields[sortField])) {
                        return 1;
                    }
                    if (Number(aa.fields[sortField]) < Number(bb.fields[sortField])) {
                        return -1;
    ...
    applyFilters() {
        ...
        for (let i = 0; i < activeTable.fullData.length; i++) {
            ...
            if (row.some(el => el !== null)) {
                newData.push({
                    index: activeTable.fullData[i].index,
                    fields: row,
                });
            }
        }
        ...
        for (let i = 0; i < activeTable.data.length; i++) {
            ...
            if (filteredIn) {
                newData.push({
                    index: activeTable.data[i].index,
                    fields: row,
                });
            }
        }
    ...
    changeTable(table, loadNew = false) {
        table.data = [];
        table.pageData = [];
        table.count = 0;
        table.page = 0;
        table.fields = table.allFields;
        if (loadNew) {
            table.sortField = 0;
    ...
    fetchData() {
        this.props.fetchData(this.state.activeTable).then((result) => {
            ...
            if (selectedFields === undefined) {
                selectedFields = activeTable.fields.map((field) => {
                    return {
                        ...field,
                        selected: true,
                    };
    ...
    render() {
        return (
            <div>
                ...
                <Modal show={this.state.showEditModal} onHide={this.hideAdd} dialogClassName="modal-1000w" backdrop="static">
