            ...
            >
                ...
                >
                    <Slider
                        ...
                        min={Number(widget.settings.min)}
                        max={Number(widget.settings.max)}
                        disabled={isRequesting}
                        value={widget.tmpValue ? widget.tmpValue : Number(widget.value)}
                        onChange={event => setWidgetValue(Number(event.target.value), widget, 'tmpValue')}
...
const CardWidget = ({ widget }) => {
    const value = Number.isNaN(Number(widget.value))
        ? widget.value : parseFloat(widget.value).toFixed(widget.settings.decimalPlaces);
    return (
        <MuiContainer
            spacing={0}
        >
...
const SwitchWidget = ({
    ...
    const [isChecked, setIsChecked] = React.useState(widget.value === widget.settings.max);
